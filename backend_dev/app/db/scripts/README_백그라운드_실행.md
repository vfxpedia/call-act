...
.\run_02_embeddings_background.bat
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd C:\Users\AI-WS01\projects\call-act\backend_dev\app\db\scripts; conda activate final_env; python 02_generate_embeddings_hana.py"

# 별도 PowerShell 창에서 로그 확인
Get-Content backend_dev\app\db\scripts\embedding_generation.log -Wait -Tail 50
```
...
# 출력을 파일로 리다이렉트
python 02_generate_embeddings_hana.py > embedding_console.log 2>&1
...
# 또는 cmd
type backend_dev\app\db\scripts\embedding_generation.log
```

### 최종 요약 확인
```powershell
# SUMMARY 섹션만 확인
Get-Content backend_dev\app\db\scripts\embedding_generation.log | Select-String "SUMMARY" -Context 0,10
```
...
```powershell
# 처리된 문서 수 확인
Get-Content backend_dev\app\db\scripts\embedding_checkpoint.json | ConvertFrom-Json
```
