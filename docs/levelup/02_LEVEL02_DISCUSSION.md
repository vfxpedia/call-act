...
- **개선 방향**:
  ...
  4. **keyword_dictionary 보강**: card_products 전용 키워드 39개 등록 (주유, 교통, 간편결제, 쇼핑 등)
- **담당**: M + B + D
- **상태**: **DB 분석 완료, 설계 진행 중**
...
## 다음 단계 (2026-02-10 16:30 갱신)

### 즉시 (금일)
| # | 담당 | 작업 | 상세 |
|---|------|------|------|
| ~~1~~ | ~~**F**~~ | ~~F-8 FeedbackModal 기본값 저장~~ | ✅ 완료 — `handleClose()`에서 `saveDefaultFeedbackScores()` 호출, 빌드 배포 완료 |
| 2 | **M** | F-8 실데이터 저장 흐름 수정 | AfterCallWorkPage → handleSaveACW에서 feedbackScores 없을 때 기본값 사용하도록 |
| 3 | **M** | M-1 STT 비교 테스트 계획 확정 | 시나리오 8종 × 3 STT 엔진, 비교 대시보드 활용 |

### 이번 주
| # | 담당 | 작업 | 상세 |
|---|------|------|------|
| 4 | **M+B** | M-1 STT 비교 테스트 실행 | WER, RTF, 키워드 보존율 측정 |
| 5 | **M+B** | M-2 키워드 추출 개선 설계 | 비정보성 발화 필터, 상담 흐름 인식 |
| 6 | **D** | D-1 fullText 전처리 계획 | DB 마크다운 포맷팅 범위/방법 수립 |
| 7 | **D** | M-2 지원: 키워드 품질 분석 | 카드 상품 49% 키워드 없음 현황 + 추출 기준 |

### 다음 주
| # | 담당 | 작업 |
|---|------|------|
| 8 | **B+D** | M-3 RAG 검색 품질 (M-1,2 결과 반영) |
| 9 | **전체** | Phase 4 논의 (admin/stats, 녹취 재생, transcript 저장) |
