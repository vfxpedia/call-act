...
export const employeesData = [
  { 
    ...
    team: '상담1팀', 
    position: '대리', 
    consultations: 127,        // 상담 건수
    fcr: 94,                   // FCR 달성률 (%)
    avgTime: '4:32',           // 평균 처리 시간
    rank: 7,                   // 전체 순위
    trend: 'up',               // 추이 ('up', 'down', 'same')
    status: 'active',          // 상태 ('active', 'inactive', 'vacation')
    joinDate: '2024-01-15',    // 입사일
...
export const consultationsData = [
  { 
    id: 'CS-20250105-1432',          // 상담 ID
    agent: '홍길동',                  // 상담사 이름
    customer: '김민수',               // 고객 이름
    category: '카드분실',             // 카테고리
    status: '완료',                   // 상태 ('완료', '진행중', '미완료')
    datetime: '2025-01-05 14:32',    // 상담 일시 (통합)
    duration: '05:12',               // 통화 시간
    isBestPractice: true,            // 우수 사례 여부
    fcr: true,                       // FCR 달성 여부
...
**주요 변경 사항:**
- ✅ `datetime` 필드로 통합 (이전: date/time 분리)
- ✅ `agent`, `customer` 필드 구분
- ✅ `isBestPractice` 필드 추가 (우수 사례 표시)
- ✅ `memo` 필드로 상담 내용 요약 (이전: title)

---

### 5.3 공지사항 데이터 (noticesData)

```typescript
export const noticesData = [
  { 
    id: 1, 
    tag: '긴급',                     // 태그 ('긴급', '시스템', '이벤트', '교육' 등)
...
### 5.4 실시간 상담 페이지 (RealTimeConsultationPage)
...
// 칸반보드 카드 (현재 상황) - 확장된 구조
interface DetailCard {
  id: number;
  title: string;
  keywords: string[];                // ['#분실신고', '#즉시정지', '#재발급']
  content: string;                   // 간단한 설명
  systemPath: string;                // ⭐ 시스템 처리 경로
  requiredChecks: string[];          // ⭐ 필수 확인 사항
  exceptions: string[];              // ⭐ 예외 케이스
  regulation: string;                // 참고 약관 제목
  detailContent: string;             // ⭐ 약관 전문
  time?: string;                     // 처리 시간
  note?: string;                     // 추가 노트
}
...
const nextStepCards: DetailCard[] = [
  {
    ...
    requiredChecks: [
      '✓ 배송 주소 확인: 등록된 주소와 일치 여부 확인',
      '✓ 배송 추적 번호 제공: 고객에게 배송 추적 번호 제공'
    ],
    exceptions: [
      '⚠️ 해외 배송: 추가 수수료 발생 (수수료 $50)'
    ],
    regulation: '카드업무 취급요령 제35조 (재발급 카드 배송)',
    detailContent: `제35조 (재발급 카드 배송)

① 재발급 카드는 신청일로부터 3-5 영업일 내 등록된 주소로 등기우편으로 발송되며, 회원은 SMS를 통해 배송 추적 번호를 제공받는다.

② 해외 배송의 경우 추가 수수료가 발생하며, 배송 시간은 7-10 영업일 내로 예상된다.

③ 배송 추적 번호는 고객에게 제공되어 배송 상태를 확인할 수 있다.

④ 배송 중 발생한 문제는 고객센터로 문의하여 즉시 해결할 수 있다.`,
    time: '처리 시간: 약 1-2분',
    note: '배송 추적 번호 제공 필수'
  }
];
...
**핵심 개선 사항:**
1. ✅ **systemPath** - 시스템 처리 경로 추가 (상담사 업무 필수)
2. ✅ **requiredChecks** - 필수 확인 사항 (상담 시 체크리스트)
3. ✅ **exceptions** - 예외 케이스 (놓치기 쉬운 특수 상황)
4. ✅ **detailContent** - 실제 약관 전문 (법적 근거)

---

### 5.5 상담 후처리 페이지 (AfterCallWorkPage)
