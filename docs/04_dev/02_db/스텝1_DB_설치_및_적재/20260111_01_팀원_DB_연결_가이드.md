...
팀장 또는 DB 호스트로부터 다음 정보를 받으세요:

- **Host**: `[팀원 IP 주소 또는 Tailscale IP]` (예: `192.168.0.100` 또는 `100.64.1.2`)
- **Port**: `5432`
- **Database**: `callact_db` ⚠️ **주의**: 언더스코어 없음 (`call_act_db` 아님)
- **Username**: `callact_admin`
- **Password**: `callact_pwd1` (또는 팀장이 공유한 비밀번호)
...
**Main 탭**:
- **Host**: 팀원 IP 주소 또는 Tailscale IP (예: `192.168.0.100`)
- **Port**: `5432`
...
#### 4.1 연결 실패: `FATAL: password authentication failed`
...
#### 4.2 연결 실패: `Connection refused` 또는 `Connection timeout`

**원인**: 
- 팀원의 Docker 서버가 실행되지 않음
- IP 주소가 잘못됨
- 방화벽 문제

**해결**:
1. 팀원에게 Docker 컨테이너 실행 확인 요청:
   ```bash
   docker ps
...
#### 4.3 `FATAL: database "callact_db" does not exist`
...
DB_PASSWORD=callact_pwd1  # 팀장이 공유한 비밀번호
DB_NAME=callact_db
```
